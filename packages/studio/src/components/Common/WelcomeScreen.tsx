  ...
  return (
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-slate-900/60">
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-lg p-8 relative">
        <button onClick={handleDismiss} className="absolute top-4 right-4 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-slate-800 dark:text-slate-100 mb-2">Welcome to RPAForge</h1>
          <p className="text-slate-500 dark:text-slate-400 text-sm leading-relaxed">
            Build powerful automation workflows visually. Drag activities onto the canvas, connect them, and run your process.
          </p>
        </div>
        <div className="space-y-3 mb-6">
          <button
            onClick={() => { handleDismiss(); onNewProcess() }}
          ...
          >
            ...
            <div>
              <div className="font-medium text-slate-800 dark:text-slate-100">Create New Process</div>
              <div className="text-xs text-slate-500">Start with a blank canvas</div>
            </div>
          </button>
          <button
            onClick={() => { handleDismiss(); onOpenProcess() }}
          ...
          >
            ...
            <div>
              <div className="font-medium text-slate-800 dark:text-slate-100">Open Existing Process</div>
              <div className="text-xs text-slate-500">Browse and open a saved process</div>
            </div>
          </button>
        </div>
